...
class FullTestPredictionCallback(TrainerCallback):
    def __init__(self, tokenizer, raw_data, output_dir, batch_size=8, max_samples=1000):
        self.tokenizer = tokenizer
        self.raw_data = raw_data
    ...
    def on_train_end(self, args, state, control, model=None, **kwargs):
        if args.local_rank not in [-1, 0]:
            return
        print("\nTraining completed. Running final prediction on full test set...")
        os.makedirs(self.output_dir, exist_ok=True)
        prediction_file = os.path.join(self.output_dir, 'final_test_predictions.json')
        if len(self.raw_data) > self.max_samples:
            print(f"Subsampling to {self.max_samples} for final prediction")
            prediction_data = self.raw_data.shuffle(seed=42).select(range(self.max_samples))
        else:
            prediction_data = self.raw_data
        results = []
        total_samples = len(prediction_data)
        print(f"Starting batched prediction on {total_samples} test samples (batch_size={self.batch_size})...")
        prompts = []
        ...
        for i in range(0, len(prompts), self.batch_size):
            ...
            for j, decoded_output in enumerate(decoded_outputs):
                ...
                if idx >= total_samples:
                    break
                data_point = prediction_data[idx]
                if "### Response:" in decoded_output:
                    predicted_output = decoded_output.split("### Response:")[-1].strip()
                else:
                    predicted_output = decoded_output.strip()
            ...
            print(f"Processed {processed}/{total_samples} samples ({processed/total_samples:.1%})")
        with open(prediction_file, 'w') as f:
            json.dump(results, f, indent=2)
        print(f"\nSaved final predictions for {len(results)} examples to {prediction_file}")
...
def train(
    ...
    val_data_path: List[str] = ["data/game/dataset/processed/test_50.json"],
    ...
    max_prediction_samples: int = 5000,
):
    params = {
    ...
    full_test_prediction = FullTestPredictionCallback(
        tokenizer=tokenizer,
        raw_data=raw_test_data,
    ...
    trainer = transformers.Trainer(
        ...
        callbacks=[early_stopping, sample_prediction, SavePeftModelCallback, full_test_prediction]
    )

...
class QwenEmbeddingCalculator:
    def __init__(self, model_path: str, batch_size: int = 8, max_length: int = 1024):
        ...
        logging.info("Qwen embedding model loaded successfully")
    ...
    def get_embeddings(self, texts: List[str]) -> torch.Tensor:
        ...
        if need_compute:
            ...
            for i in range(0, len(need_compute), self.batch_size):
                ...
                batch_embeddings = hidden_states[torch.arange(hidden_states.size(0)), last_token_indices, :].detach().cpu()
                embeddings.append(batch_embeddings)
            if embeddings:
                all_embeddings = torch.cat(embeddings, dim=0)
                for j, text in enumerate(need_compute):
                    self.embedding_cache[text] = all_embeddings[j]
        final_embeddings = []
        compute_idx = 0
        for i in range(len(texts)):
            if i in need_compute_idx:
                final_embeddings.append(self.embedding_cache[need_compute[compute_idx]])
                compute_idx += 1
            else:
...
class RewardCalculator:
    def __init__(self, qwen_model_path: str):
        self.embedding_calculator = QwenEmbeddingCalculator(qwen_model_path)
...
def train_grpo(
    ...
    base_model_path: str = "base_models/Qwen2.5-1.5B-Instruct",
    sft_lora_weights: str = "sft/Qwen2.5-1.5B-Instruct-game-base-0/checkpoint-32",
    ...
    output_dir: str = "./Qwen2.5-1.5B-Instruct-grpo-0-0-2",
    seed: int = 0,
    batch_size: int = 8,
    num_epochs: int = 2,
    ...
    reward_calculator = RewardCalculator(base_model_path)
